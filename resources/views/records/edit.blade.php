...
<div class="container mx-auto mt-10">
    ...
    <form action="{{ route('records.update', $record['id']) }}" method="POST" class="bg-white shadow-md rounded p-6">
        ...
        <div class="mb-4">
            ...
            <input type="text" name="name" id="name" class="form-control"value="{{ $record['fields']['Name'] }}" required>
        </div>

        <div class="mb-4">
            <label for="description" class="form-label text-gray-700">Description</label>
            <textarea name="description" id="description" class="form-control">{{ $record['fields']['Description'] }}</textarea>
        </div>

        <div class="mb-4">
            <label for="status" class="form-control">Status</label>
            <select name="status" id="status" class="w-full px-4 py-2 border rounded">
                <option value="Todo" {{ $record['fields']['Status'] == 'Todo' ? 'selected' : '' }}>Todo</option>
                <option value="Done" {{ $record['fields']['Status'] == 'Done' ? 'selected' : '' }}>Done</option>
            </select>
        </div>
